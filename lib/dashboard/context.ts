...
export async function getDashboardContext() {
  ...
  const { data: tenant } = await supabase
    .from("tenants")
    .select(
      "id, name, slug, system_prompt, ai_model, message_limit, current_month_messages, counter_period_start, modules, is_active, logo_url, brand_color, plan, monthly_fee, billing_due_date, billing_status, addon_price, addon_enabled, abandoned_carts_enabled, service_paused"
    )
